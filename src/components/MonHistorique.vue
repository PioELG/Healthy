<template>
    ...
    <header style="margin-left:20px">
        <h5><b><i class="fa fa-history fa-fw"></i> MonHistorique</b> 
          <router-link to="/AjouterModelM"><i class="fa fa-plus fa-fw" style="margin-left:800px" ></i> </router-link>
        </h5>  
    </header>
      ...
      <div v-for="medicament in medicaments" :key="medicament.id" style="margin-left:45px; margin-top:20px;margin-right:45px; " class="medoc">
        <p><strong>Vous avez prescrit du {{ medicament.nom }}  à {{ getNomPrenom(medicament.patient_id).nom }} {{ getNomPrenom(medicament.patient_id).prenom }} le {{medicament.datePresc}}</strong> </p>
  ...
  export default {
    ...
    methods: {
      ...
      try {
        const response = await axios.get('http://192.168.224.1:8080/api/medicament/medecin',config);
        this.medicaments = response.data;

        
        axios.get('http://192.168.224.1:8080/api/malade', config) // Utilisez la configuration avec l'en-tête d'autorisation
